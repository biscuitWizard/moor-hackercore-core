object PROG_FEATURE
  ...
  verb "@prop*erty" (any any any) owner: #2 flags: "rxd"
    set_task_perms(player);
    nargs = length(args);
    ...
    perms = nargs < 3 ? "r" | $perm_utils:apply("r", args[3]);
    if (nargs < 4)
      owner = $hacker;
    else
    ...
    try
      ...
      player:notify(tostr("Property ", name, " added to ", $su:nn(object), " with value '", toliteral(object.(name)), "'."));
      $vcs:update(object);
      $broadcast:staff_alerts($su:nn(player), " added @property ", name, " to ", $su:nn(object), " with value '", toliteral(object.(name)), "'.");
    except (E_INVARG)
  ...
  verb "@count-code @cc" (any any any) owner: #2 flags: "rxd"
    if (!args)
      return player:tell("Usage: ", verb, " <object>");
    endif
    object = player:match(argstr);
    if (object == $failed_match)
      return player:tell("I don't see anything called '", argstr, "'.");
    endif
    verbs = length(verbs(object));
    code = 0;
    for x in [1..verbs]
      code = code + length(verb_code(object, x));
    endfor
    player:tell(object.name:capitalize(), " has ", verbs, " ", $s("verb", verbs), " => ", code, " lines of code.");
  endverb
endobject