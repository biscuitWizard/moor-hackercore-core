object COMMAND_UTILS
  ...
  verb object_match_failed (this none this) owner: #2 flags: "rxd"
    ...
    if ($recycler:valid(match_result))
      return $false;
    elseif (!string || match_result == $nothing)
      player:system_tell("You must give the name of some object.");
    elseif (match_result == $failed_match)
      player:system_tell(tostr("I see nothing named \"", string, "\" here."));
    elseif (match_result == $ambiguous_match)
      player:system_tell(tostr("I don't know which \"", string, "\" you mean."));
    else
      player:system_tell(tostr(string, " does not exist."));
    endif
    return $true;
  endverb
