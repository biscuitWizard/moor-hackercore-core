object PLAYER
  ...
  property out_of_band_session (owner: #2, flags: "r") = NOTHING;
  ...
  verb notify (this none this) owner: HACKER flags: "rxd"
  ...
  verb "help @help help/* @help/*" (any any any) owner: HACKER flags: "rd"
  ...
  verb "system_tell systell" (this none this) owner: HACKER flags: "rxd"
    return this:notify(tostr(@args));
  endverb

  verb "x*" (any any any) owner: HACKER flags: "rd"
    command = verb[2..$];
    if (!command)
      return player:system_tell("Use x<channel> to speak!");
    elseif (!(channel_match = $match_utils:match_list(command, $ou:descendants($channel))))
      return player:system_tell(tostr("Unable to find a channel with the alias '", command, "'."));
    endif
    channel = channel_match[1];
    if (!channel:is_on_channel(player))
      return player:system_tell("You're not on that channel. Type '@xjoin ", channel.aliases[1], "' to join it.");
    elseif (reason = channel:why_cant_transmit(player))
      return player:system_tell(reason);
    endif
    channel:transmit(tostr(channel:player_title(player), player:name(), ": ", argstr));
  endverb

  verb "@xjoin @xlisten" (any any any) owner: HACKER flags: "rd"
    if (!argstr)
      return player:system_tell("Type @xjoin <channel> to join that channel, or @channels for a list of all channels to join.");
    elseif (!(channel_match = $match_utils:match_list(argstr, $ou:descendants($channel))))
      return player:system_tell(tostr("Unable to find a channel with the alias '", argstr, "'."));
    elseif (!channel_match[1]:can_join(player))
      return player:system_tell(tostr("Unable to find a channel with the alias '", argstr, "'."));
    endif
    channel = channel_match[1];
    if (channel:is_on_channel(player))
      return player:system_tell("You're already on that channel.");
    endif
  ...
  verb "@xleave @xsil*ence" (any any any) owner: HACKER flags: "rd"
    if (!argstr)
      return player:system_tell("Type @", verb, " <channel> to silence that channel.");
    elseif (!(channel_match = $match_utils:match_list(argstr, $ou:descendants($channel))))
      return player:system_tell(tostr("Unable to find a channel with the alias '", argstr, "'."));
    endif
    channel = channel_match[1];
    if (!channel:is_on_channel(player))
      return player:system_tell("You're not on that channel.");
    endif
    player:system_tell("You leave the ", channel.name, " channel.");
    player.channels = setremove(player.channels, channel);
    channel:nrtransmit(tostr(player:name(), " has left the channel."));
  endverb

  verb "@channels" (none none none) owner: HACKER flags: "rd"
    lines = {tostr(" ", $su:left("CHANNEL", 26))};
    lines = {@lines, $su:space(27, "-")};
    for channel in ($ou:descendants($channel))
      if ($ou:is_generic(channel))
        continue;
      elseif (!channel:can_join(player))
        continue;
      endif
      lines = {@lines, tostr(" ", $su:left($ansi:((channel.color))(channel:name()), 26))};
    endfor
    lines = {@lines, "Type @xjoin <channel> to join that channel."};
    player:notify_lines(lines);
  endverb

  verb less_ascii (this none this) owner: HACKER flags: "rxd"
    return $false;
  endverb

  verb "ex*amine" (any none none) owner: HACKER flags: "rxd"
    if (!dobjstr)
